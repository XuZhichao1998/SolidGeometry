...
int main()
{
	...
	cout<<"\n许智超设计/**这个程序目的是求四棱截锥体ABCD-A1B1C1D1体积。\n先把四棱锥分为两个三棱锥。可用大三棱锥体积减去小三棱锥的体积的求出答案**/\n-------------------------\n";
	cout<<"请输入空间点A,A1，B,B1，C,C1，D,D1的坐标和顶点O的坐标：\n";
	while(scanf("%lf%lf%lf%lf%lf%lf%lf%lf%lf%lf%lf%lf%lf%lf%lf%lf%lf%lf%lf%lf%lf%lf%lf%lf%lf%lf%lf",&x1,&y1,&z1,&x2,&y2,&z2,&x3,&y3,&z3,&x4,&y4,&z4,&x5,&y5,&z5,&x6,&y6,&z6,&x7,&y7,&z7,&x8,&y8,&z8,&x0,&y0,&z0)!=EOF)
	{
		...
		if(flag3*flag4)
		{
			...
			s=s2-s1+s4-s3;
			printf("大三棱锥O-ABC体积为%.6lf\n小三棱锥0-A1B1C1体积为：%.6lf\n大三棱锥O-BCD体积为%.6lf\n小三棱锥0-B1C1D1体积为：%.6lf\n",s2,s1,s4,s3);
			printf("该四棱截锥体体积为: %.6lf-%.6lf+%.6lf-%.6lf=%.6lf\n",s2,s1,s4,s3,s);
			cout<<"\n------------------------------------------------------\n请继续输入空间点A,A1，B,B1，C,C1,D,D1的坐标：\n";
